...
const EditAssignment = ({navigation, route}: {navigation: any; route: any}) => {
  const currentAssignmentdata = route?.params?.currentAssignment;
  ...
  const [valueClass, setValueClass] = useState(currentAssignmentdata?.class_id);
  ...
  const [valueSubject, setValueSubject] = useState(
    currentAssignmentdata?.subject_id,
  );
  ...
  const [name, setName] = useState(currentAssignmentdata?.name);
  const [totalMark, setTotalMark] = useState(
    currentAssignmentdata?.total_mark?.toString(),
  );
  ...
  const [description, setDescription] = useState(
    currentAssignmentdata?.description,
  );
      ...
      <ScrollView
        ...
        <View
          ...
          <DateComp
            ...
            DateName="Deadline"
            selectedDate={currentAssignmentdata?.assignment_deadline}
        ...
        <ButtonComp
          styleBtn={{marginHorizontal: wp('5%'), bottom: hp('2%')}}
          btnName={
            isLoading ? (
